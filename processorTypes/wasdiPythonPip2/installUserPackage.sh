...
function showHelp() {
    ...
    echo
    echo "Usage: /bin/bash ${0} --home-directory <directory> [--package-apt] [--package-conda] [--package-pip] [--venv-directory <directory>] [--failure-is-ok]"
    echo
    echo "If --package-pip is provided, --venv-directory becomes mandatory."
    echo
    echo "Return code:"
    echo -e "\t- if no --package-* argument are provided: the script exit with a return code 1"
...
function cleanCacheApt() {
    ...
    echo "[INFO] Clean the APT cache deleting '/var/lib/apt/lists/*'..."
    ...
    then
        ...
        echo -e "[ERROR] Please analyse the logs"
    fi
...
function cleanCachePip() {
    ...
    echo "[INFO] Search '__pycache__' directories in '${venvDirectory}' and delete them..."
    find ${venvDirectory} -xdev -type d -name __pycache__ -exec rm -rf {} +
    returnCode=${?}
...
if [[ -n "${installUserPackagePip}" ]]
then
    if [[ -z "${venvDirectory}" ]]
    then
        echo "[ERROR] Please specify the venv directory in which user libraries are installed."
        errorDetected="true"
    fi

    if [[ -n "${venvDirectory}" && ! -d "${venvDirectory}" ]]
    then
        echo "[INFO] The directory '${venvDirectory}' doesn't exists."
        errorDetected="true"
    fi
fi
...
then
    echo "==== APT ===="
    installPackageApt
    returnCodeInstallationApt=${?}

    if [[ "${failureIsOk}" == "true" ]]
    then
        returnCodeInstallationApt=0
    fi
...
then
    echo "==== CONDA ===="
    installPackageConda
    returnCodeInstallationConda=${?}

    if [[ "${failureIsOk}" == "true" ]]
    then
        returnCodeInstallationConda=0
    fi
...
then
    echo "==== PIP ===="
    installPackagePip
    returnCodeInstallationPip=${?}

    if [[ "${failureIsOk}" == "true" ]]
    then
        returnCodeInstallationPip=0
    fi
