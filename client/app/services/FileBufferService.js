...
service('FileBufferService', ['$http',  'ConstantsService', function ($http, oConstantsService) {
    ...
    this.download = function(sUrl, sWorkspaceId,sBounds,sProvider) {
        ...
        return this.m_oHttp.get(this.APIURL + '/filebuffer/download?fileUrl='+sEncodedUri+"&workspaceId="+sWorkspaceId+"&bbox="+sBounds+'&provider='+sProvider);
    }
    
    this.publishBand = function(sUrl, sWorkspaceId, sBand) {
        return this.m_oHttp.get(this.APIURL + '/filebuffer/publishband?fileUrl='+encodeURIComponent(sUrl)+"&workspaceId="+sWorkspaceId+'&band='+sBand);
    }
