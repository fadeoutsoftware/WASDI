...
class Wasdi {
    ...
    printStatus() {
        ...
        console.log("[INFO] jswasdilib.printStatus: parameters file path: " +
        ...
        console.log("[INFO] jswasdilib.printStatus: workspace base url: " +
    ...
    asyncLoadConfig(filename) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let result = yield initPromiseChain;
            return result;
        });
    }
    /**
   ...
   */
    ...
    getProductsByActiveWorkspace() {
        ...
        console.log("[INFO] jswasdilib.productListByActiveWorkspace: Products in the active workspace ");
        productList.forEach((a) => console.log(a.name));
        return productList;
    }
    getProductsByWorkspace(workspaceName) {
        return this.getProductsByWorkspaceId(this.getWorkspaceIdByName(workspaceName));
    }
    /**
     * Retrieve a list of workspace of the current logged user.
     */
    getProductsByWorkspaceId(workspaceId) {
        let productList = this.getObject(this._m_sBaseUrl + "/product/byws", "?workspace=" + workspaceId);
        console.log("[INFO] jswasdilib.productListByWorkspace: Products in the active workspace ");
        productList.forEach((a) => console.log(a.name));
        return productList;
    }
    /**
    ...
    deleteProduct(sProduct) {
        let response = this.getObject(this._m_sBaseUrl +
            "/product/delete?name=" +
            sProduct +
        ...
        if (response === null || response === void 0 ? void 0 : response.ok) {
            console.log("[INFO] jswasdilib.deleteProduct: Product " + sProduct + " deleted");
        }
        else {
            console.log("[ERROR] jswasdilib.deleteProduct: Can't delete product " +
                sProduct +
                " in workspace " +
                this._m_sActiveWorkspace);
        }
    ...
    openWorkspaceById(workspaceID) {
        ...
        if (ws.workspaceId) {
            ...
            console.log("[INFO] jswasdilib.openWorkspace: Opened Workspace " +
                this._m_sWorkspaceName);
            return ws;
        }
        else {
    ...
    openWorkspace(workspaceName) {
        ...
        if (wsList) {
            ...
            if (ws.workspaceId) {
                console.log("[INFO] jswasdilib.openWorkspaceByName: Opened Workspace " +
                    ws.workspaceName);
                return this.openWorkspaceById(ws.workspaceId);
            }
            else {
                console.log('[INFO] jswasdilib.openWorkspaceByName: Could not find workspace "' +
                    workspaceName +
                    '"');
            }
        }
    ...
    executeProcessor(appname, jsonParameters) {
        if (this._m_sActiveWorkspace) {
            let response = this.postObject(this._m_sWorkspaceBaseUrl + "/processors/run", "?name=" + appname + "&workspace=" + this._m_sActiveWorkspace, jsonParameters);
    ...
    setProcessPayload(sProcessId, data) {
        return this.getObject(this._m_sWorkspaceBaseUrl + "/process/setpayload", "?procws=" + sProcessId + "&payload=" + encodeURIComponent(data));
    }
    /**
    ...
    createWorkspace(wsName) {
        ...
        if (ws.stringValue) {
            console.log("[INFO] jswasdilib.CreateWorkspace: New workspace " +
                wsName +
                " created");
    ...
    getProcessStatus(processId) {
        ...
        if (procWs.status) {
            console.log("[INFO] jswasdilib.getProcessStatus: Process status " +
                procWs.status +
                " Percentage " +
                procWs.progressPerc +
                " %");
    ...
    getLayerWMS(sProductName, sBand) {
        ...
        while (!(sourceMapPayload === null || sourceMapPayload === void 0 ? void 0 : sourceMapPayload.layerID)) {
            sourceMapPayload = this.publishBand(sProductName, sBand);
            this.delay(2000);
        }
        let ret = {
            server: sourceMapPayload === null || sourceMapPayload === void 0 ? void 0 : sourceMapPayload.geoserverUrl,
            "layerId ": sourceMapPayload === null || sourceMapPayload === void 0 ? void 0 : sourceMapPayload.layerId,
        };
        return ret;
    }
    /**
     ...
     */
    getProcessesByWorkspace(sWorkspaceId, sOperationType, sNamePattern, dateFrom, dateTo, iStartIndex, iEndIndex) {
        return this.getObject(this._m_sBaseUrl + "/process/byws", "?workspace=" +
            sWorkspaceId +
            "&operationType=" +
            sOperationType +
            "&namePattern=" +
     ...
     * @returns
     */
    getProcessesByApp(sProcessorName) {
        return this.getObject(this._m_sBaseUrl + "/process/byapp", "?processorName=" + sProcessorName);
    }
    /**
    ...
    publishBand(fileName, bandName) {
        ...
        let file = this.getProductsByActiveWorkspace().find((a) => a.fileName == fileName);
        if (file && file.bandsGroups) {
            ...
            if (band) {
                ...
                let published = this.getObject(this._m_sBaseUrl + "/filebuffer/publishband", "?fileUrl=" +
                    fileName +
                    "&workspace=" +
                    this._m_sActiveWorkspace +
                    "&band=" +
            ...
            console.log("[INFO] jswasdilib.publishBand: File " +
                fileName +
