function [asProductNames, asProductLinks, asProductFootprints]=wSearchEOImages(Wasdi, sPlatorm, sDateFrom, sDateTo, dULLat, dULLon, dLRLat, dLRLon, sProductType, iOrbitNumber, sSensorOperationalMode, sCloudCoverage);
...
%   asProductFootprints: array of strings that are the footprints of found products in WKT
  ...
  if exist("Wasdi") < 1 
   ...
   aoProducts = Wasdi.searchEOImages(sPlatform, sDateFrom, sDateTo, dULLat, dULLon, dLRLat, dLRLon, sProductType, iOrbitNumber, sSensorOperationalMode, sCloudCoverage);
  ...
  for iProduct = 0:iTotFound-1
    ...
    asProductNames{iProduct+1} = oWorkspace.get("title");
    asProductLinks{iProduct+1} = oWorkspace.get("link");
	asProductFootprints{iProduct+1} = oWorkspace.get("footprint");
  end
