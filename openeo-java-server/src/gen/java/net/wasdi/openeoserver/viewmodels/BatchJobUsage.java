...
/**
 * Metrics about the resource usage of the batch job.  Back-ends are not expected to update the metrics while processing data, so the metrics can only be available after the job has been finished or has errored. For usage metrics during processing, metrics can better be added to the logs (e.g. &#x60;GET /jobs/{job_id}/logs&#x60;) with the same schema.
 */
@ApiModel(description = "Metrics about the resource usage of the batch job.  Back-ends are not expected to update the metrics while processing data, so the metrics can only be available after the job has been finished or has errored. For usage metrics during processing, metrics can better be added to the logs (e.g. `GET /jobs/{job_id}/logs`) with the same schema.")
@JsonPropertyOrder({
  BatchJobUsage.JSON_PROPERTY_CPU,
  BatchJobUsage.JSON_PROPERTY_MEMORY,
  BatchJobUsage.JSON_PROPERTY_DURATION,
  BatchJobUsage.JSON_PROPERTY_NETWORK,
  BatchJobUsage.JSON_PROPERTY_DISK,
  BatchJobUsage.JSON_PROPERTY_STORAGE
})
...
public class BatchJobUsage   {
  ...
  @Valid 
  public UsageCpu getCpu() {
    return cpu;
  }

  public void setCpu(UsageCpu cpu) {
    this.cpu = cpu;
  }

  public BatchJobUsage memory(UsageMemory memory) {
    this.memory = memory;
    return this;
  }
  ...
  @ApiModelProperty(value = "")
  @Valid 
  public UsageMemory getMemory() {
    return memory;
  }

  public void setMemory(UsageMemory memory) {
    this.memory = memory;
  }

  public BatchJobUsage duration(UsageDuration duration) {
  ...
  public void setDuration(UsageDuration duration) {
    this.duration = duration;
  }

  public BatchJobUsage network(UsageNetwork network) {
  ...
  public BatchJobUsage disk(UsageDisk disk) {
    this.disk = disk;
    return this;
  }
  ...
  @ApiModelProperty(value = "")
  @Valid 
  public UsageDisk getDisk() {
    return disk;
  }

  public void setDisk(UsageDisk disk) {
    this.disk = disk;
  }

  public BatchJobUsage storage(UsageStorage storage) {
  ...
  public boolean equals(Object o) {
    ...
    BatchJobUsage batchJobUsage = (BatchJobUsage) o;
    return Objects.equals(this.cpu, batchJobUsage.cpu) &&
        Objects.equals(this.memory, batchJobUsage.memory) &&
        Objects.equals(this.duration, batchJobUsage.duration) &&
        Objects.equals(this.network, batchJobUsage.network) &&
        Objects.equals(this.disk, batchJobUsage.disk) &&
        Objects.equals(this.storage, batchJobUsage.storage);
  }

  @Override
  public int hashCode() {
    return Objects.hash(cpu, memory, duration, network, disk, storage);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class BatchJobUsage {\n");
