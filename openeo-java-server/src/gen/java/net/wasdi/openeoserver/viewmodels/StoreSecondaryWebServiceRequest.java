 ...
 */
@JsonPropertyOrder({
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_TITLE,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_DESCRIPTION,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_PROCESS,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_TYPE,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_ENABLED,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_CONFIGURATION,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_PLAN,
  StoreSecondaryWebServiceRequest.JSON_PROPERTY_BUDGET
})
...
public class StoreSecondaryWebServiceRequest   {
  ...
  @ApiModelProperty(value = "Describes whether a secondary web service is responding to requests (true) or not (false). Disabled services don't produce any costs.")
  ...
  @ApiModelProperty(example = "{\"version\":\"1.3.0\"}", value = "Map of configuration settings, i.e. the setting names supported by the secondary web service combined with actual values. See `GET /service_types` for supported configuration settings. For example, this could specify the required version of the service, visualization details or any other service dependant configuration.")
  ...
  /**
   * The billing plan to process and charge the job or service with.  Billing plans MUST be accepted in a *case insensitive* manner. If &#x60;null&#x60; is given implicitly (through the default value) or explicitly by the client, the server MUST store the default plan and persist it regardless of any changes to the default billing plan in the future. If given, the default plan specific to the user has higher priority than the default plan of the back-end.  The list of plans and the default plan for the back-end can be retrieved from &#x60;GET /&#x60;. The default plan for the user can be retrieved from &#x60;GET /me&#x60;. Billing plans not on the list of available plans MUST be rejected with openEO error &#x60;BillingPlanInvalid&#x60;.
   * @return plan
   **/
  @JsonProperty(value = "plan")
  @ApiModelProperty(example = "free", value = "The billing plan to process and charge the job or service with.  Billing plans MUST be accepted in a *case insensitive* manner. If `null` is given implicitly (through the default value) or explicitly by the client, the server MUST store the default plan and persist it regardless of any changes to the default billing plan in the future. If given, the default plan specific to the user has higher priority than the default plan of the back-end.  The list of plans and the default plan for the back-end can be retrieved from `GET /`. The default plan for the user can be retrieved from `GET /me`. Billing plans not on the list of available plans MUST be rejected with openEO error `BillingPlanInvalid`.")
