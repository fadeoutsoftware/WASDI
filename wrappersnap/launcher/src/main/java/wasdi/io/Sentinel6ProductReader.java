...
public class Sentinel6ProductReader extends SnapProductReader {
	...
	public String adjustFileAfterDownload(String sDownloadedFileFullPath, String sFileNameFromProvider) {
		...
		try {
			...
			WasdiLog.debugLog("Sentinel6ProductReader.adjustFileAfterDownload: File is a Sentinel-6 image, start unzip");
...
//			deleteDownloadedZipFile(sDownloadedFileFullPath);
			...
			if (oNCFile != null) {
				WasdiLog.debugLog("Sentinel6ProductReader.adjustFileAfterDownload: nc file found at path: " + oNCFile.getAbsolutePath());
			} else {
				WasdiLog.debugLog("Sentinel6ProductReader.adjustFileAfterDownload: no nc file found in folder: " + sNewFolderPath);
			}
			

			m_oProductFile = new File(sNewFolderPath); // here we change the pointer to the file, from the zip file to the _MTL.txt file, which provides the information for reading the bands
