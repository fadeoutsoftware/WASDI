...
public abstract class DownloadFile {
     ...
     */
    public DownloadFile() {
		m_oLogger = LauncherMain.s_oLogger;
	}
    ...
    public void UpdateProcessProgress(ProcessWorkspace oProcessWorkspace, int iProgress) {
        ...
        try {
	        ...
	        if (!oProcessWorkspaceRepository.UpdateProcess(oProcessWorkspace))
	            m_oLogger.debug("LauncherMain.DownloadFile: Error during process update with process Perc");
	
	        //send update process message
	        if (LauncherMain.s_oSendToRabbit != null) {
				if (!LauncherMain.s_oSendToRabbit.SendUpdateProcessMessage(oProcessWorkspace)) {
					m_oLogger.debug("LauncherMain.DownloadFile: Error sending rabbitmq message to update process list");
				}	        	
	        }
		} catch (Exception oEx) {
			m_oLogger.error("LauncherMain.DownloadFile: Exception " + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(oEx));
			oEx.printStackTrace();
		}
    }
