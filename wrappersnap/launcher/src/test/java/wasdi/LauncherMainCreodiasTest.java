...
public class LauncherMainCreodiasTest extends LauncherMainTest {
	...
	private String extractFileNameFromUrl(String sUrl) {
		...
		try {
			String[] asParts = sUrl.split(DiasResponseTranslatorCREODIAS.SLINK_SEPARATOR_CREODIAS);
			String sFileName = asParts[DiasResponseTranslatorCREODIAS.IPOSITIONOF_FILENAME];
			return sFileName;
		} catch (Exception oE) {
...
//	@Test
	...
	private String extractFilePathFromUrl(String sUrl) {
		...
		try {
			...
			String sFilePath = asParts[DiasResponseTranslatorCREODIAS.IPOSITIONOF_PRODUCTIDENTIFIER];
