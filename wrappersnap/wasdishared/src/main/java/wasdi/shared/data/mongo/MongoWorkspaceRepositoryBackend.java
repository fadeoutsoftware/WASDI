...
/**
 * Mongo backend implementation for workspace repository.
 */
public class MongoWorkspaceRepositoryBackend extends MongoRepository implements IWorkspaceRepositoryBackend {

	public MongoWorkspaceRepositoryBackend() {
	...
	public Long getStorageUsageForUser(String sUserId) {
		...
		try {
			List<Document> aoPipeline = Arrays.asList(
					new Document("$match", new Document("userId", sUserId)),
					new Document("$group", new Document()
							.append("_id", null)
							.append("totalStorage", new Document("$sum",
									new Document("$toLong",
											new Document("$ifNull", Arrays.asList("$storageSize", 0))
									)
							))
					)
