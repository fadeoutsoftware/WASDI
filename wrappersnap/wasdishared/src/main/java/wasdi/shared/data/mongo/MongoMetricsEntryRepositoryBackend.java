...
/**
 * Mongo backend implementation for metrics entry repository.
 */
public class MongoMetricsEntryRepositoryBackend extends MongoRepository implements IMetricsEntryRepositoryBackend {

	public MongoMetricsEntryRepositoryBackend() {
	...
	public MetricsEntry getLatestMetricsEntryByNode(String sNode) {
		...
		try {
			Document oDocument = getCollection(m_sThisCollection).find(new Document("node", sNode)).sort(new Document("timestamp", -1)).first();
