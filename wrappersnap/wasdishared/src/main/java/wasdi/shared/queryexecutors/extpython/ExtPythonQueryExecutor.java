package wasdi.shared.queryexecutors.extpython;
...
public class ExtPythonQueryExecutor extends QueryExecutor {
	...
	public void init() {
		...
		WasdiLog.debugLog("ExtPythonQueryExecutor:  parser config path is " + m_sParserConfigPath );
		
		try {
			m_sPythonScript = m_oDataProviderConfig.pythonScript;
			WasdiLog.debugLog("ExtPythonQueryExecutor: python script path " + m_sPythonScript);
			
			m_sExchangeFolder = WasdiConfig.Current.paths.wasdiTempFolder;
			WasdiLog.debugLog("ExtPythonQueryExecutor: exchange folder: " + m_sExchangeFolder);
		}
		catch (Exception oEx) {
			WasdiLog.errorLog("ExtPythonQueryExecutor: exception reading config files ", oEx);
		}		
	}
	...
	protected List<String> getCommandLineArgs(String sCommand, String sQuery) {
		...
		try {
			...
			WasdiLog.errorLog("ExtPythonQueryExecutor.getCommandLineArgs: error ",oEx);
		}
	...
	public int executeCount(String sQuery) {
		...
		try {
			...
			if (asArgs == null) {
				WasdiLog.errorLog("ExtPythonQueryExecutor.executeCount: no args!!");
				return -1;
			}
			
			if (asArgs.size()<5) {
				WasdiLog.errorLog("ExtPythonQueryExecutor.executeCount: not enough args");
				return -1;				
			}
			...
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeCount: python output = = " + oShellExecReturn.getOperationLogs());
			
			
			File oOutputFile = new File(sOutputFullPath);
			
			if (!oOutputFile.exists()) {
				WasdiLog.warnLog("ExtPythonQueryExecutor.executeCount: impossible to read the output from python script");
				return -1;
			}
			
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeCount: got output file");
			
			JSONObject oOutput = JsonUtils.loadJsonFromFile(sOutputFullPath);
			
			int iCount = oOutput.getInt("count");
			
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeCount: return count " + iCount);
	...
	public List<QueryResultViewModel> executeAndRetrieve(PaginatedQuery oQuery, boolean bFullViewModel) {
		...
		try {
			...
			if (asArgs == null) {
				WasdiLog.errorLog("ExtPythonQueryExecutor.executeAndRetrieve: no args!!");
				return aoReturnList;
			}
			
			
			if (asArgs.size()<5) {
				WasdiLog.errorLog("ExtPythonQueryExecutor.executeAndRetrieve: not enough args");
				return aoReturnList;				
			}
			...
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeAndRetrieve: python output = " + oShellExecReturn.getOperationLogs());
			
			
			File oOutputFile = new File(sOutputFullPath);
			
			if (!oOutputFile.exists()) {
				WasdiLog.warnLog("ExtPythonQueryExecutor.executeAndRetrieve: impossible to read the output from python script");
				return aoReturnList;
			}
			
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeAndRetrieve: got output file");
			...
			WasdiLog.debugLog("ExtPythonQueryExecutor.executeAndRetrieve: return elements list " + aoReturnList.size() );
