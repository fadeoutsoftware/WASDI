...
public class ResponseTranslatorCreoDias2 extends ResponseTranslator {
	...
	public List<QueryResultViewModel> translateBatch(String sResponse, boolean bFullViewModel) {
		...
		if (Utils.isNullOrEmpty(sResponse)) {
			WasdiLog.debugLog("ResponseTranslatorCreoDias2.translateBatch. The response string is null or empty.");
			return aoResults;
		}
	...
	private QueryResultViewModel processProduct(JSONObject oJsonItem, boolean bFullViewModel) {
		...
		// some properties to add to the summary: instrument, mode, platform name and serial id, relative orbit
	 ...
	 */
	private void setBasicInfo(QueryResultViewModel oViewModel, String sProductId, String sProductTitle, String sSummary, String sFootprint) {
	...
	private void setLink(QueryResultViewModel oResult, String sDownloadLink, String sProductTitle, double dSizeInBytes, String sS3Path) {
		...
		try {
			...
			sTitle = sTitle + ".zip";
			oLink.append(sTitle).append(SLINK_SEPARATOR_CREODIAS2); 
			...
			oLink.append(sSizeInBytes).append(SLINK_SEPARATOR_CREODIAS2); 
			...
			oLink.append(sPathIdentifier).append(SLINK_SEPARATOR_CREODIAS2); 
